...
from app.api.v1.schemas.endpoints import EndpointFileRequest
...
from app.api.v1.schemas.endpoints import EndpointSuccessResponse
...
async def get_endpoint_file(project_id: str, endpoint_path: str, method: str,  db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        request = EndpointFileRequest(project_id=project_id, endpoint_path=endpoint_path, method=method)
...
@router.post("/", response_model=EndpointSuccessResponse, status_code=status.HTTP_201_CREATED)
...
async def delete_endpoint_file(
    project_id: str = Query(..., description="Project ID returned from project initialization"),
    endpoint_path: str = Query(..., description="Path of the endpoint to delete (e.g., 'api/v1/endpoint')"),
    method: str = Query(..., description="HTTP method for this endpoint (GET, POST, PUT, DELETE)"),
    description: Optional[str] = Query(None, description="Optional description of the endpoint"),
