...
class MigrationRunData(BaseModel):
    ...
    success: bool = Field(..., description="Indicates if the migrations were applied successfully")
    message: str = Field(..., description="Details about the migration execution")
    database_path: Optional[str] = Field(None, description="Path to the SQLite database file")
