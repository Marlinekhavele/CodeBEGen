        ...
        try:
            ...
            if needs_models:
                ...
                if update_result and update_result.get("model_updated", False):
                    ...
                    model_code = update_result.get(
                        "model_code", "# Updated model code not available"
                    )
                    ...
                    result["model"] = {
                        "file_path": update_result.get(
                            "model_file",
                            f"models/{entity_name.lower()}{LangchainService.get_file_extension(language)}",
                        ),
                        "generated_code": model_code,
                        "content_base64": update_result.get(
                            "content_base64",
                            LangchainService.encode_content(model_code),
                        ),
                        "file_hash": update_result.get(
                            "file_hash", LangchainService.generate_file_hash(model_code)
                        ),
                        "exists": True,
                        "updated": True,
                        "entity_name": entity_name,
                    ...
                    if update_result.get("schema_updated", False):
                        ...
                        placeholder_schema = (
                            f"# No updates needed for the schema of {entity_name}"
                        )
                        result["schema"] = {
