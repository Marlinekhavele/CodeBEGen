...
class CodeGenerationService:
    ...
    async def _notify_event(self, callback: Optional[EventCallback], event_name: str, data: Any):
    ...
    async def generate_endpoint(self, 
        ...
        try:
            ...
            await self._notify_event(self.on_endpoint_start, "endpoint", {
    ...
    async def generate_model(self,
                            ...
                            language: str = "python",
                            endpoint_code: Optional[str] = None) -> Dict[str, Any]:
        """Generate model code with event notifications"""
        try:
            # Notify model generation start
            await self._notify_event(self.on_model_start, "model", {
    ...
    async def generate_schema(self,
                             ...
                             model_code: Optional[str] = None) -> Dict[str, Any]:
        """Generate schema code with event notifications"""
        try:
            # Notify schema generation start
            await self._notify_event(self.on_schema_start, "schema", {
                "entity_name": entity_name
            })
    ...
    async def generate_migration(self,
        ...
        try:
            ...
            await self._notify_event(self.on_migration_start, "migration", {
                "entity_name": entity_name
            })
    ...
    async def generate_helpers(self,
                              project_id: str,
                              entity_name: str,
                              entity_description: str,
                              language: str = "python",
                              ...
                              schema_code: Optional[str] = None) -> Dict[str, Any]:
        """Generate helper functions with event notifications"""
        try:
            # Notify helpers generation start
            await self._notify_event(self.on_helpers_start, "helpers", {
                "entity_name": entity_name
            })
    ...
    async def generate_code(self, request: CodeGenerationRequest) -> CodeGenerationResponse:
        ...
        try:
            ...
            await self._notify_info(f"Analysis complete. Database components needed: {needs_models}")
            ...
            if needs_models:
                ...
                await self._notify_info("Generating additional components (model, schema, helpers)")
    ...
    async def _check_and_update_existing_model(self, project_id, entity_name, prompt_description, endpoint_code, language):
        ...
        try:
            ...
            if not existing_model:
                logger.info(f"No existing model found for {entity_name}, will generate new components")
                return None
        ...
        if model and not model.get("exists", False):
            try:
                ...
                if migration and migration.get("generated_code"):
                    ...
                    logger.info(f"Committed migration file: {migration.get('file_path')}")
            except Exception as e:
