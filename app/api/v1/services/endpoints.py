...
class EndpointService:
    @staticmethod
    async def get_file(request: EndpointFileRequest, db: Session = Depends(get_db)) -> EndpointFileResponse:
        """
        ...
        if not project:
            logger.error(f"GET FILE: Project with slug {request.project_id} not found in database")
        ...
        logger.info(f"GET FILE: Found endpoint in database: {endpoint is not None}, description: {description}")
    ...
    # Rest of the method remains the same...
        ...
        try:
            ...
            file_path = resolve_file_path(project_dir, request.endpoint_path, request.method)
    ...
    async def create_endpoint(
        ...
        try:
            ...
            try:
                ...
                # Get the commit hash
                logger.info(f"CREATE FILE: Getting commit hash")
                commit_hash = get_latest_commit_hash(project_dir)
                logger.info(f"CREATE FILE: Commit hash: {commit_hash}")

                # Push changes
                logger.info(f"CREATE FILE: Pushing changes to remote repository")
                ...
                if db:
                    try:
                        logger.info(f"CREATE FILE: Saving endpoint to database")
                        
                        # Get the project by slug to get its UUID
                        project = db.query(Project).filter(Project.slug == request.project_id).first()
                        
                        if not project:
                            logger.error(f"CREATE FILE: Project with slug {request.project_id} not found in database")
                        else:
                            # Use the project's UUID as the project_id for the endpoint
                            project_uuid = project.id
                            logger.info(f"CREATE FILE: Found project UUID: {project_uuid} for slug: {request.project_id}")
                            
                            # Create new endpoint record with the project UUID
                            new_endpoint = EndPoint(
                                path=request.endpoint_path,  # Changed from name to path
                                method=method if method else "UNKNOWN",  # Changed from http_method to method
                                project_id=project_uuid,  # Use UUID here, not slug
                                description=request.description,
                                file_hash=hashlib.md5(content.encode()).hexdigest()  # Add file_hash

                            )
                            # Add to database
                            db.add(new_endpoint)
                            db.commit()
                            logger.info(f"CREATE FILE: Successfully saved endpoint to database with project_id: {project_uuid}")
                    except Exception as db_error:
                        logger.error(f"CREATE FILE: Database operation failed: {str(db_error)}")
    ...
    async def update_file(request: EndpointFileRequest) -> EndpointFileResponse:
        ...
        try:
            ...
            try:
                ...
                # Get the commit hash
                logger.info(f"UPDATE FILE: Getting commit hash")
                commit_hash = get_latest_commit_hash(project_dir)
                logger.info(f"UPDATE FILE: Commit hash: {commit_hash}")

                # Push changes
                logger.info(f"UPDATE FILE: Pushing changes to remote repository")
                push_result = run_git_command(["git", "push"], cwd=project_dir)
    ...
    @staticmethod
    async def delete_file(request: EndpointFileRequest, db: Session = None) -> EndpointFileResponse:
        """
        ...
        if EndpointService.is_critical_endpoint(request.endpoint_path):
            logger.warning(f"DELETE FILE: Attempt to delete critical endpoint {request.endpoint_path}")
        ...
        try:
            ...
            relative_path = str(file_path.relative_to(project_dir)) if file_path.exists() else None
            ...
            if db:
                try:
                    ...
                    if not project:
                        logger.error(f"DELETE FILE: Project with slug {request.project_id} not found in database")
                        if transaction:
                    ...
                    if not file_path.exists() and not endpoint:
                        logger.warning(f"DELETE FILE: Neither file nor database entry exists for {request.endpoint_path}")
                        if transaction:
                            transaction.rollback()
                        return EndpointFileResponse(
                            project_id=request.project_id,
                    ...
                    logger.error(f"DELETE FILE: Database lookup failed: {str(db_error)}")
                    if transaction:
                        transaction.rollback()
            ...
            if db and endpoint_id:
                try:
                    logger.info(f"DELETE FILE: Processing endpoint {endpoint_id} deletion in database")
                    ...
                    if endpoint:
                        if use_soft_delete and hasattr(endpoint, 'deleted_at'):
                            ...
                            logger.info(f"DELETE FILE: Endpoint soft-deleted (marked as deleted)")
                        else:
                            # Hard delete - permanently remove from database
                            db.delete(endpoint)
                            logger.info(f"DELETE FILE: Endpoint hard-deleted from database")
                    else:
                        logger.warning(f"DELETE FILE: Endpoint not found in database")
                        
                except Exception as db_error:
                    logger.error(f"DELETE FILE: Database deletion failed: {str(db_error)}")
                    if transaction:
                        transaction.rollback()
                    raise ValueError(f"Database error: {str(db_error)}")
            ...
            if file_path.exists() or relative_path:
                try:
                    logger.info(f"DELETE FILE: Adding file deletion to Git index")
                    run_git_command(
                    ...
                    commit_message = f"feat: Delete {method} endpoint {request.endpoint_path}"
                    logger.info(
                    ...
                    # Get the commit hash
                    logger.info(f"DELETE FILE: Getting commit hash")
                    commit_hash = get_latest_commit_hash(project_dir)
                    logger.info(f"DELETE FILE: Commit hash: {commit_hash}")

                    # Push changes
                    logger.info(f"DELETE FILE: Pushing changes to remote repository")
                    push_result = run_git_command(["git", "push"], cwd=project_dir)
                    logger.info(f"DELETE FILE: Push result: {push_result}")
                except Exception as git_err:
                    logger.error(f"DELETE FILE: Git operation failed: {str(git_err)}")
                    # Continue with operation even if Git fails
            
            # Commit the transaction if everything succeeded
            if transaction:
                transaction.commit()
                db.commit()
                logger.info(f"DELETE FILE: Database transaction committed successfully")
