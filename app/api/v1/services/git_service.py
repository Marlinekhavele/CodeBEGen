...
from app.api.v1.utils.endpoint_services import get_project_dir_from_repo_url, resolve_file_path
...
class GitService:
    @staticmethod
    async def commit_code_update(project_id: str, new_code: str, endpoint_path: str = None, method: str = None) -> str:
        """
        ...
        try:
            ...
            logger.info(f"Committing code update for project {project_id} with repo URL: {repo_url}")
            ...
            run_git_command(["git", "add", str(file_path.relative_to(project_dir))], cwd=project_dir)
            
            # Commit the changes
            commit_message = f"Update code in {file_path.relative_to(project_dir)}"
            logger.info(f"Committing file to Git repository with message: {commit_message}")
            run_git_command(["git", "commit", "-m", commit_message], cwd=project_dir)
            
            # Get the new commit hash
            logger.info(f"Getting commit hash")
            commit_hash = run_git_command(["git", "rev-parse", "HEAD"], cwd=project_dir)
            logger.info(f"Commit hash: {commit_hash}")
            
            # Push changes
            logger.info(f"Pushing changes to remote repository")
    ...
    @staticmethod
    async def commit_file_update(project_id: str, new_code: str, file_path: str, commit_message: str = None) -> str:
        """
        ...
        try:
            # Get the repository URL for the project
            repo_url = get_repo_url(project_id)
            logger.info(f"Committing file update for project {project_id} with repo URL: {repo_url}")
