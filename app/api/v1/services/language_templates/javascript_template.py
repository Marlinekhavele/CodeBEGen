...
class JavaScriptTemplate(LanguageTemplate):
    ...
    def extract_entity_from_code(self, code: str) -> Optional[str]:
        ...
        model_import = re.search(r"(const|let|var)\s+(\w+)\s*=\s*require\(['\"]\.\.\/models\/(\w+)['\"]", code)
        if model_import:
            return model_import.group(3)
            
        # ES6 import pattern
        es6_import = re.search(r"import\s+(\w+)\s+from\s+['\"]\.\.\/models\/(\w+)['\"]", code)
        if es6_import:
        ...
        **kwargs
        ...
        if component_type in template_map:
            ...
            template_vars = {
                ...
                "endpoint_code": kwargs.get("endpoint_code", "") or kwargs.get("controller_code", ""),
                "model_code": kwargs.get("model_code", ""),
                "schema_code": kwargs.get("schema_code", ""),
                "latest_migration_id": kwargs.get("latest_migration_id", "migration_" + datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
            }
    ...
    def get_commit_strategy(self) -> Dict[str, Any]:
        ...
        return {
            "components": ["model", "controller", "validation", "utils", "route", "migration"],
            "commit_order": ["model", "validation", "utils", "controller", "route", "migration"],
            "commit_messages": {
