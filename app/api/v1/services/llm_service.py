...
class LLMService:
    ...
    async def generate_complete_endpoint(
        ...
        path_to_use = (
            endpoint_path or f"api/v1/{endpoint_description.lower().replace(' ', '_')}"
        )

        try:
            # Step 1: Generate the endpoint and identify entity (using Langchain)
            logger.info(
                f"Step 1: Generating {language} endpoint for: {endpoint_description}"
            )
            ...
            try:
                ...
                if language == "python":
                    ...
                    dependencies_v1, primary_entity_ast_v1 = (
                        visitor_v1.get_analysis_results()
                    )
                    logger.info(
                        f"AST Analysis Results - Primary Entity Guess: {primary_entity_ast_v1}"
                    )
                    logger.info(
                        f"AST Analysis Results - Dependencies: {dependencies_v1}"
                    )
                else:
                    ...
                    from app.api.v1.utils.entity_extractor import (
                        extract_entity_from_code,
                    )

                    primary_entity_ast_v1 = extract_entity_from_code(
                        endpoint_result["generated_code"], language
                    )
                    logger.info(
                        f"Entity extraction result for {language}: {primary_entity_ast_v1}"
                    )
            except SyntaxError as e:
                logger.error(
                    f"AST Parsing failed for generated {language} code: {e}. Cannot perform dependency analysis."
                )
            except Exception as e:
                logger.error(
                    f"Unexpected error during {language} code analysis: {e}",
                    exc_info=True,
                )
            # --- End AST Analysis ---
            ...
            if primary_entity_ast_v1:
                ...
                logger.warning(
                    f"Code analysis failed or yielded no entity name. Using final fallback: '{entity_name}'"
                )

            # Ensure entity_name is assigned
            if not entity_name:
                logger.error(
                    "CRITICAL: Entity name determination failed completely. Defaulting to 'User'."
                )
                entity_name = "User"
            ...
            if existing_model:
                ...
                logger.info(
                    f"Checking if existing model needs updates for: {entity_name}"
                )
                ...
                if update_result.get("model_updated", False):
                    ...
                    logger.info(
                        f"Updated existing model {entity_name} with new fields: {update_result.get('fields_added', [])}"
                    )
                    return {
                    ...
                    logger.info(
                        f"Using existing model for entity: {entity_name} (no updates needed)"
                    )
                    return {
                        "endpoint": endpoint_result,
            ...
            logger.info(
                f"Step 2: Generating {language} model for entity: {entity_name}"
            )
            ...
            logger.info(
                f"Step 3: Generating {language} schema for entity: {entity_name}"
            )
            ...
            logger.info(
                f"Step 4: Generating {language} migration for entity: {entity_name}"
            )
            ...
            logger.info(
                f"Step 5: Generating {language} helper functions for entity: {entity_name}"
            )
    ...
    def _find_existing_model(
        entity_name: str, models: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
        """Check if a model with the given entity name already exists"""
        if not entity_name or not models:
            logger.info("No entity name provided or no models available to search")
            return None

        entity_lower = entity_name.lower()
        logger.info(
            f"Looking for model matching entity: '{entity_name}' (lowercase: '{entity_lower}')"
        )
        ...
        for model in models:
            ...
            logger.info(
                f"Comparing with model: '{model.get('name')}' (lowercase: '{model_name}')"
            )

            # Check for direct match
            if model_name == entity_lower:
                logger.info(
                    f"✅ MATCH FOUND: Exact match with model '{model.get('name')}'"
                )
                return model
            ...
            if model_name in entity_variations or any(
                ...
                logger.info(
                    f"✅ MATCH FOUND: Variation match with model '{model.get('name')}'"
                )
                return model
            ...
            if entity_lower in model_name or model_name in entity_lower:
                logger.info(
                    f"✅ MATCH FOUND: Substring match with model '{model.get('name')}'"
                )
                return model

        logger.warning(
            f"❌ NO MATCH FOUND: No model found matching entity '{entity_name}'"
        )
        return None
