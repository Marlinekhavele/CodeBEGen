...
class JavaScriptModelUpdater(ModelUpdater):
    ...
    def parse_model_structure(self, model_code: str, entity_name: str) -> Dict[str, Any]:
        """
        ...
        is_sequelize = "sequelize" in model_code.lower() or "datatypes" in model_code.lower()
        
        structure = {
            "model_type": "mongoose" if is_mongoose else "sequelize" if is_sequelize else "unknown",
            "schema_definition": {
                "start_pos": -1,
                "end_pos": -1
            },
        ...
        # Parse imports
        import_regex = re.compile(r'(const|let|var|import)\s+([^=;]+)(?:=\s*require\([\'"]([^\'"]+)[\'"]\)|from\s+[\'"]([^\'"]+)[\'"])')
    ...
    def _parse_mongoose_structure(self, model_code: str, entity_name: str, structure: Dict[str, Any]):
        ...
        if schema_match:
            ...
            field_regex = re.compile(r'([A-Za-z0-9_]+)\s*:\s*({[\s\S]*?})(?:,|$)', re.MULTILINE)
    ...
    def _parse_sequelize_structure(self, model_code: str, entity_name: str, structure: Dict[str, Any]):
    ...
    def _parse_generic_js_structure(self, model_code: str, entity_name: str, structure: Dict[str, Any]):
        """Parse a generic JavaScript model structure as fallback"""
        # Look for object definitions that might be models
        object_regex = re.compile(r'(?:const|let|var)\s+([A-Za-z0-9_]+)\s*=\s*({[\s\S]*?});', re.MULTILINE)
        ...
        for match in object_regex.finditer(model_code):
            ...
            if any(variation in obj_name for variation in entity_variations):
                ...
                field_regex = re.compile(r'([A-Za-z0-9_]+)\s*:\s*([^,}]+)(?:,|$)', re.MULTILINE)
    ...
    def update_model(self, model_path: Path, entity_name: str, 
                     field_changes: List[Dict[str, Any]], 
                     existing_model_code: str) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        if structure["model_type"] == "mongoose":
            return self._update_mongoose_model(structure, field_changes, existing_model_code)
        elif structure["model_type"] == "sequelize":
            return self._update_sequelize_model(structure, field_changes, existing_model_code)
        else:
            # Generic JS model update as fallback
            return self._update_generic_js_model(structure, field_changes, existing_model_code)
    
    def _update_mongoose_model(self, structure: Dict[str, Any], 
                               field_changes: List[Dict[str, Any]], 
                               existing_model_code: str) -> Tuple[str, Dict[str, Any]]:
        ...
        for change in field_changes:
            if change["type"].lower() == "remove":
                ...
                if field_name in structure["fields"]:
                    ...
                    # Find the full field entry including commas
                    ...
                    if field_match:
                        ...
                        updated_code = updated_code[:field_match.start()] + updated_code[field_match.end():]
                        ...
                        logger.warning(f"Could not locate field {field_name} in the Mongoose schema")
                else:
                    logger.warning(f"Cannot remove field {field_name}: not found in Mongoose schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "rename":
                ...
                if field_name in structure["fields"]:
                    ...
                    if field_match:
                        # Rename the field
                        updated_code = updated_code[:field_match.start(1)] + new_name + updated_code[field_match.end(1):]
                    ...
                    logger.warning(f"Cannot rename field {field_name}: not found in Mongoose schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "modify":
                ...
                if field_name in structure["fields"]:
                    # Find the field in the schema
                    field_def = structure["fields"][field_name]["definition"]
                    field_pattern = re.escape(field_name) + r'\s*:\s*(' + re.escape(field_def) + r')'
                    field_match = re.search(field_pattern, updated_code)
                    
                    if field_match:
                        # Replace the field definition
                        updated_code = updated_code[:field_match.start(1)] + change["definition"] + updated_code[field_match.end(1):]
                        modified_fields.append(field_name)
                        
                        # Update the structure
                        structure["fields"][field_name]["definition"] = change["definition"]
                    else:
                        logger.warning(f"Could not locate field definition for {field_name} in the Mongoose schema")
                else:
                    logger.warning(f"Cannot modify field {field_name}: not found in Mongoose schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "add":
                ...
                if field_name in structure["fields"]:
                    logger.warning(f"Field {field_name} already exists in Mongoose schema")
                    continue
                ...
                updated_code = updated_code[:insertion_pos] + new_field_entry + updated_code[insertion_pos:]
        ...
        changes_made = len(added_fields) + len(modified_fields) + len(removed_fields) + len(renamed_fields) > 0
        change_summary = {
