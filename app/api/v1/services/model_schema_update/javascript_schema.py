...
class JoiSchemaUpdater(SchemaUpdater):
    ...
    def find_schemas(self, schema_content: str, entity_name: str) -> List[Dict[str, Any]]:
        """
        ...
        for match in schema_pattern.finditer(schema_content):
            ...
            if any(variation in schema_name.lower() for variation in entity_variations):
                ...
                field_pattern = re.compile(r'([A-Za-z0-9_]+)\s*:\s*Joi\.([\s\S]*?)(?:,|$)', re.MULTILINE)
                fields = {}
    ...
    def update_schema(self, schema_content: str, schema_info: Dict[str, Any], 
                      field_changes: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for change in field_changes:
            if change["type"].lower() == "remove":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    field_pattern = re.escape(field_name) + r'\s*:\s*' + re.escape(field_def) + r'\s*,?'
                    field_match = re.search(field_pattern, updated_content)
                    
                    if field_match:
                        # Remove the field
                        updated_content = updated_content[:field_match.start()] + updated_content[field_match.end():]
                        updated = True
                    else:
                        logger.warning(f"Could not locate field {field_name} in the Joi schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "rename":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    if field_match:
                        # Rename the field
                        updated_content = updated_content[:field_match.start(1)] + new_name + updated_content[field_match.end(1):]
                        updated = True
                    else:
                        logger.warning(f"Could not locate field {field_name} in the Joi schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "modify":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    field_pattern = field_name + r'\s*:\s*(' + re.escape(field_def) + r')'
                    field_match = re.search(field_pattern, updated_content)
                    
                    if field_match:
                        # Replace the field definition
                        updated_content = updated_content[:field_match.start(1)] + change["definition"] + updated_content[field_match.end(1):]
                        updated = True
                    else:
                        logger.warning(f"Could not locate field definition for {field_name} in the Joi schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "add":
                ...
                updated_content = updated_content[:insertion_pos] + new_field_entry + updated_content[insertion_pos:]
                updated = True
    ...
    def convert_model_changes_to_schema_changes(self, model_changes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for change in model_changes:
            ...
            if "definition" in change:
                ...
                new_change["definition"] = self._convert_model_to_joi(change["definition"])
    ...
    def _convert_model_to_joi(self, model_definition: str) -> str:
        ...
        if "type: String" in model_definition:
            ...
            type_str = "binary()"
        elif "type: ObjectId" in model_definition or "type: mongoose.Schema.Types.ObjectId" in model_definition:
            ...
            type_str = "string()"
        elif "DataTypes.INTEGER" in model_definition or "DataTypes.BIGINT" in model_definition:
            type_str = "number().integer()"
        elif "DataTypes.FLOAT" in model_definition or "DataTypes.DOUBLE" in model_definition or "DataTypes.DECIMAL" in model_definition:
            type_str = "number()"
        ...
        if "required: true" in model_definition.lower() or "allowNull: false" in model_definition.lower():
            required = True
    ...
    def find_schemas(self, schema_content: str, entity_name: str) -> List[Dict[str, Any]]:
        ...
        for match in schema_pattern.finditer(schema_content):
            ...
            if any(variation in schema_name.lower() for variation in entity_variations):
                ...
                field_pattern = re.compile(r'(?:check|body|param|query)\([\'"]([A-Za-z0-9_]+)[\'"]\)([\s\S]*?)(?:,|\n|$)', re.MULTILINE)
                fields = {}
    ...
    def update_schema(self, schema_content: str, schema_info: Dict[str, Any], 
            ...
            if change["type"].lower() == "remove":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    if field_match:
                        ...
                        logger.warning(f"Could not locate field {field_name} in the Express-validator schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "rename":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    updated_content_new = re.sub(field_pattern, r'\1' + new_name + r'\2', updated_content)
            ...
            if change["type"].lower() == "modify":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    if field_match:
                        ...
                        updated_content = updated_content[:field_match.start()] + new_def + updated_content[field_match.end():]
                        updated = True
                    else:
                        logger.warning(f"Could not locate field definition for {field_name} in the Express-validator schema")
        ...
        for change in field_changes:
            if change["type"].lower() == "add":
                ...
                updated_content = updated_content[:schema_end_pos] + new_def + '\n  ' + updated_content[schema_end_pos:]
                updated = True
    ...
    def convert_model_changes_to_schema_changes(self, model_changes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for change in model_changes:
            ...
            if "definition" in change:
                ...
                new_change["definition"] = self._convert_model_to_express_validator(change["field_name"], change["definition"])
                
            schema_changes.append(new_change)
            
        return schema_changes
        
    def _convert_model_to_express_validator(self, field_name: str, model_definition: str) -> str:
        """
        ...
        elif "type: Number" in model_definition or "DataTypes.INTEGER" in model_definition or "DataTypes.FLOAT" in model_definition:
            validator += ".isNumeric().withMessage('Must be a number')"
        elif "type: Boolean" in model_definition or "DataTypes.BOOLEAN" in model_definition:
        ...
        elif "type: Array" in model_definition or "type: []" in model_definition or "DataTypes.ARRAY" in model_definition:
            validator += ".isArray().withMessage('Must be an array')"
        elif "type: Object" in model_definition or "type: {}" in model_definition or "DataTypes.JSON" in model_definition:
    ...
    def find_schemas(self, schema_content: str, entity_name: str) -> List[Dict[str, Any]]:
        ...
        for match in schema_pattern.finditer(schema_content):
            ...
            if any(variation in schema_name.lower() for variation in entity_variations):
                ...
                field_pattern = re.compile(r'([A-Za-z0-9_]+)\s*:\s*([^,}]+)(?:,|$)', re.MULTILINE)
                fields = {}
            ...
            if change["type"].lower() == "remove":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    if field_match:
                        ...
                        logger.warning(f"Could not locate field {field_name} in the JavaScript schema")
            ...
            if change["type"].lower() == "modify":
                ...
                if field_name in schema_info["fields"]:
                    ...
                    if field_match:
                        ...
                        logger.warning(f"Could not locate field definition for {field_name} in the JavaScript schema")
