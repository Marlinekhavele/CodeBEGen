...
class PydanticSchemaUpdater(SchemaUpdater):
    ...
    def find_schemas(self, schema_content: str, entity_name: str) -> List[Dict[str, Any]]:
        ...
        class_pattern = re.compile(r'class\s+([A-Za-z0-9_]+)\s*\(\s*[A-Za-z0-9_.]+\s*\)\s*:', re.MULTILINE)
        ...
        for match in class_pattern.finditer(schema_content):
            ...
            if any(variation in class_name.lower() for variation in entity_variations):
                ...
                if class_line_num < len(lines):
                    ...
                    field_pattern = re.compile(r'^\s+([A-Za-z0-9_]+)\s*(?::|=)', re.MULTILINE)
                    fields = {}
        ...
        for change in field_changes:
            if change["type"].lower() == "add":
                ...
                if schema_info["fields"]:
                    last_field_line = max(field_info["line"] for field_info in schema_info["fields"].values())
    ...
    def convert_model_changes_to_schema_changes(self, model_changes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for change in model_changes:
            ...
            if "definition" in change:
                ...
                new_change["definition"] = self._convert_sqlalchemy_to_pydantic(change["definition"])
