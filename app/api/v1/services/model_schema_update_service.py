...
class ModelSchemaManager:
    ...
    async def process_model_changes(
        ...
        try:
            ...
            logger.info(f"Project analysis found models: {[m.get('name') for m in project_analysis.get('models', [])]}")
            ...
            if schema_results.get("schema_updated", False):
                for schema_result in schema_results.get("schema_results", []):
                    if schema_result.get("updated", False):
                        ...
                        if schema_content:
                            logger.info(f"Adding schema to commit: schemas/{schema_result.get('schema_file')} with {len(schema_content)} bytes")
                            files_to_commit.append({
                            ...
                            logger.warning(f"Skipping schema commit for {schema_result.get('schema_file')} - no content available")
            
            logger.info(f"Prepared {len(files_to_commit)} files to commit: {[f['file_path'] for f in files_to_commit]}")
                ...
                for change in changes:
                    ...
                    if change_type == "add" or change_type == "modify":
                        if "definition" not in change:
                            logger.warning(f"Skipping {change_type} operation missing definition: {change}")
                            continue
                    elif change_type == "rename":
                        if "new_name" not in change:
                            logger.warning(f"Skipping rename operation missing new_name: {change}")
                            continue
                    elif change_type != "remove":
                        logger.warning(f"Skipping unknown operation type: {change_type}")
                        continue
                    
                    # Skip standard fields
                    if change["field_name"].lower() in ["id", "created_at", "updated_at"]:
                        logger.warning(f"Skipping change to standard field: {change['field_name']}")
                        continue
    ...
    def _update_model_file(
        ...
        try:
            ...
            model_structure = ModelSchemaManager._parse_model_structure(existing_model_code, entity_name)
            ...
            with open(model_path, 'r') as source_file, open(temp_file_path, 'w') as target_file:
                ...
                for line in source_file:
                    ...
                    if not in_model_class and re.search(f"class\\s+{entity_name}\\s*\\(", line_stripped):
                        in_model_class = True
                    ...
                    if in_model_class and (line_stripped.startswith("class ") or current_line_idx == model_structure["class_end_line"]):
                    ...
                    if field_match:
                        ...
                        if not modified:
                            for rename_change in changes_by_type["rename"]:
                                if rename_change["field_name"] == field_name and field_name not in processed_renames:
                    ...
                    if in_model_class and not additions_inserted:
                        ...
                        if (current_line_idx == model_structure["fields_end_line"] or
                            current_line_idx == model_structure["insertion_line"]):
            ...
            change_summary = {
                ...
                "added_fields": [change["field_name"] for change in changes_by_type["add"]],
                "modified_fields": [change["field_name"] for change in changes_by_type["modify"]],
                "removed_fields": [change["field_name"] for change in changes_by_type["remove"]],
                "renamed_fields": [
                    {"old_name": change["field_name"], "new_name": change["new_name"]} 
                    for change in changes_by_type["rename"]
                ],
                "diff": ModelSchemaManager._generate_diff(existing_model_code, updated_content)
            }
    ...
    def _parse_model_structure(model_code: str, entity_name: str) -> Dict[str, Any]:
        ...
        has_timestamps = "created_at" in result["existing_fields"] or "updated_at" in result["existing_fields"]
    ...
    async def update_schemas(
        ...
        try:
            ...
            pydantic_changes = ModelSchemaManager._convert_to_pydantic_changes(field_changes)
                ...
                try:
                    ...
                    if updated:
                        ...
                        results.append({
                            ...
                            "diff": ModelSchemaManager._generate_diff(original_schema, updated_schema),
                    ...
                    logger.error(f"Error updating schema {schema_file}: {str(e)}", exc_info=True)
                    # Restore from backup
                    with open(schema_path, 'w') as f:
                        f.write(original_schema)
    ...
    @staticmethod
    def _find_schema_classes(schema_content: str, entity_name: str) -> List[Dict[str, Any]]:
        """
