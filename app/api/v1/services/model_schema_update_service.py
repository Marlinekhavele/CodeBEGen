...
class ModelSchemaManager:
    ...
    @staticmethod
    def _convert_to_pydantic_changes(
        sqlalchemy_changes: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """
        ...
        for change in sqlalchemy_changes:
            ...
            if "definition" in change:
                pydantic_change["definition"] = (
                    ModelSchemaManager._convert_to_pydantic_type(change["definition"])
                )
                pydantic_changes.append(pydantic_change)
    ...
    def _update_schema_class(
        ...
        for change in field_changes:
            ...
            if change_type in changes_by_type:
                ...
                if (
                    change_type == "add"
                    and schema_type == "create"
                    and change.get("definition", "").startswith("Optional[")
                ):
                    # Make it non-optional for Create schemas
                    non_optional_def = re.sub(
                        r"Optional\[(.*?)\]", r"\1", change["definition"]
                    )
        ...
        if schema_type == "update":
            ...
            for change in list(changes_by_type["add"]):
                ...
                if change.get("definition") and not change["definition"].startswith(
                    "Optional["
                ):
                    ...
                    if is_field_def and current_field:
                        ...
                        for mod_change in changes_by_type["modify"]:
                            if mod_change["field_name"] == current_field:
                                ...
                                if "=" in line_stripped:
                                    default_value = line_stripped.split("=", 1)[
                                        1
                                    ].strip()
