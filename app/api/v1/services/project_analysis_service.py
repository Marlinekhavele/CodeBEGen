...
import re
import ast
from app.api.v1.utils.git_operations import REPOS_DIR
...
class ProjectAnalysisService:
    ...
    async def analyze_project(project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if endpoints_dir.exists():
                analysis["endpoints"] = ProjectAnalysisService._analyze_endpoints(endpoints_dir)
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    if '.' in file_stem:
                        ...
                        if len(parts) >= 2 and parts[-1].lower() in ['get', 'post', 'put', 'delete', 'patch']:
                    ...
                    method_from_code = ProjectAnalysisService._extract_method_from_content(content)
                    
                    # Use method from filename if available, otherwise from code
                    method = method_from_filename or method_from_code
                    
                    # Extract function name and parameters
                    function_info = ProjectAnalysisService._extract_function_info(content)
                    ...
                    crud_ops = ProjectAnalysisService._analyze_crud_operations(content, method)
                ...
                if isinstance(node, ast.FunctionDef):
                    for arg in node.args.args:
                        if arg.annotation and isinstance(arg.annotation, ast.Name):
                            ...
                            if arg.annotation.id not in ['str', 'int', 'float', 'bool', 'dict', 'list', 'Any', 'Session']:
    ...
    @staticmethod
    def _analyze_crud_operations(content: str, method: Optional[str]) -> Dict[str, bool]:
        """
    ...
    def _analyze_models(models_dir: Path) -> List[Dict[str, Any]]:
        ...
        for root, _, files in os.walk(models_dir):
            for file in files:
                if file.endswith('.py') and file != '__init__.py':
                    ...
                    is_model_file = "sqlalchemy" in content or "Column" in content or "Base" in content
                    
                    # Add all classes from possible model files to ensure we don't miss any
                    if is_model_file:
                        for class_name in class_names:
                            logger.info(f"Found potential model class: {class_name} in {file}")
                            models.append({
                                "name": class_name,
                                "file": file,
                                "table_name": class_name.lower() + "s",  # Default convention
                        ...
                        for node in ast.walk(tree):
                            if isinstance(node, ast.ClassDef):
                                ...
                                for base in node.bases:
                                    if isinstance(base, ast.Name) and base.id in ["Base", "Model", "db.Model"]:
                                        is_model = True
                                        break
                                ...
                                if not is_model:
                                    ...
                                    for item in node.body:
                                        if (isinstance(item, ast.Assign) and 
                                            ...
                                            getattr(item.value.func, 'id', '') == 'Column'):
                                            is_model = True
                                            break
                                ...
                                if is_model:
                                    ...
                                    already_added = any(m['name'] == class_name and m['file'] == file for m in models)
                                    
                                    if already_added:
                                        # Update the existing entry with more details
                                        for model in models:
                                            if model['name'] == class_name and model['file'] == file:
                                                ...
                                                for class_item in node.body:
                                                    ...
                                                    if (isinstance(class_item, ast.Assign) and 
                                                        len(class_item.targets) == 1 and 
                                                        isinstance(class_item.targets[0], ast.Name) and
                                                        class_item.targets[0].id == "__tablename__"):
                                                        
                                                        if isinstance(class_item.value, ast.Str):
                                                            table_name = class_item.value.s
                                                    
                                                    # Look for field assignments with Column
                                                    if (isinstance(class_item, ast.Assign) and 
                                                        len(class_item.targets) == 1 and 
                                                        isinstance(class_item.targets[0], ast.Name)):
                                                        
                                                        field_name = class_item.targets[0].id
                                                        
                                                        # Check if it's a Column definition
                                                        is_column = False
                                                        if (isinstance(class_item.value, ast.Call) and 
                                                            isinstance(class_item.value.func, ast.Name) and
                                                            class_item.value.func.id == "Column"):
                                                            is_column = True
                                        ...
                                        # Add as a new model
                                        logger.info(f"Found SQLAlchemy model via AST: {class_name} in {file}")
                                        models.append({
                                            "name": class_name,
                                            "file": file,
                                            "table_name": class_name.lower() + "s",  # Default convention
        ...
        logger.info(f"Found a total of {len(models)} models: {[m['name'] for m in models]}")
        return models
    ...
    def _generate_summary(analysis: Dict[str, Any]) -> str:
        ...
        if analysis.get("endpoints"):
            endpoints_summary = f"The project has {len(analysis['endpoints'])} endpoints:"
        ...
        if analysis.get("helpers"):
            helpers_summary = f"The project has {len(analysis['helpers'])} helper functions:"
