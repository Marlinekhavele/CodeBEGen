...
class DependencyVisitor(ast.NodeVisitor):
    ...
    def _add_candidate_score(self, entity_name: str, score: int):
        ...
        if entity_name and entity_name != 'Base': # Ignore Base model/schema
            ...
            if base_name and not base_name.isdigit():
                self.primary_entity_candidates[base_name] = self.primary_entity_candidates.get(base_name, 0) + score
            else:
                logger.debug(f"Skipping invalid candidate score addition for: '{entity_name}' -> '{base_name}'")
        ...
        if node.module:
            ...
            if first_part == 'models' and len(module_parts) == 2:
                ...
                for alias in node.names:
                    ...
                    self._add_candidate_score(alias.name.replace("Schema", "").replace("Create", "").replace("Update", ""), 1)
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == 'query':
             ...
             if isinstance(base_obj, ast.Name) and base_obj.id == 'db':
                 if node.args and isinstance(node.args[0], ast.Name):
                     ...
                     self._add_candidate_score(model_name, 5) # Score 5 for direct query usage
                 ...
                 if decorator.func.attr in ('get', 'post', 'put', 'delete', 'patch'):
                     for keyword in decorator.keywords:
                         if keyword.arg == 'response_model':
                             schema_name = self._extract_schema_name_from_node(keyword.value)
                             if schema_name:
                                 # Infer entity name (e.g., User from UserSchema)
                                 entity_name = schema_name.replace("Schema", "")
                                 self._add_candidate_score(entity_name, 5) # Score 5 for response_model usage
    ...
    def _extract_schema_name_from_node(self, node: ast.AST) -> Optional[str]:
         ...
         if isinstance(node, ast.Name):
             return node.id
         elif isinstance(node, ast.Subscript) and isinstance(node.value, ast.Name) and node.value.id in ('List', 'Optional'):
             ...
             elif isinstance(slice_node, ast.Subscript): # Handle nested types like Optional[List[X]]
    ...
    def get_analysis_results(self) -> Tuple[Dict[str, Set[str]], Optional[str]]:
        ...
        if self.primary_entity_candidates:
            ...
            try:
                primary_entity = max(self.primary_entity_candidates, key=self.primary_entity_candidates.get)
                # Basic validation/cleanup
                if not primary_entity or not primary_entity.isalnum():
                    logger.warning(f"Primary entity candidate '{primary_entity}' failed validation.")
                    primary_entity = None # Discard if it looks invalid
            except ValueError: # Handles case where candidates might be empty after filtering
                 logger.warning("No valid primary entity candidates found after scoring.")
                 primary_entity = None
