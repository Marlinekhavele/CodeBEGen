...
def extract_entity_from_code(code: str, language: str = "python") -> Optional[str]:
    ...
    try:
        ...
        logger.error(
            f"Error extracting entity from {language} code: {e}", exc_info=True
        )
        return None
...
def extract_entity_from_javascript(code: str) -> Optional[str]:
    ...
    model_import = re.search(
        r'const\s+(\w+)\s+=\s+require\([\'"]\.\.\/models\/(\w+)[\'"]', code
    )
    if model_import:
    ...
    helper_import = re.search(
        r'const\s+{\s*.*\s*}\s+=\s+require\([\'"]\.\.\/helpers\/(\w+)Helpers[\'"]', code
    )
    if helper_import:
...
def extract_entity_from_java(code: str) -> Optional[str]:
    ...
    if method_params:
        ...
        if (
            not param_type.startsWith("String")
            and not param_type.startsWith("int")
            and not param_type.startsWith("boolean")
        ):
            return param_type
