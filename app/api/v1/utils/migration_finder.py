...
def get_latest_migration_id(alembic_dir: str = "alembic") -> str:
    ...
    try:
        ...
        logger.warning(f"Could not determine latest migration using Alembic API: {str(e)}")
    ...
    try:
        ...
        logger.debug(f"Found {len(migration_files)} migration files in versions directory")
        
        # Log the first few files for debugging
        if migration_files:
            sample = migration_files[:5] if len(migration_files) > 5 else migration_files
            ...
            with open(file_path, 'r') as f:
                ...
                if rev_match:
                    ...
                    if down_rev_match:
                        ...
                        # Store for debugging
                        revision_ids[rev_id] = down_rev.replace("'", "").replace('"', "") if down_rev != "None" else None
        ...
        logger.error(f"Could not determine latest migration using file parsing: {str(e)}", exc_info=True)
