...
def test_delete_endpoint_file_invalid_request(client):
    ...
    missing_params = [error["loc"][-1] for error in error_details if error["type"] == "missing"]
    assert "endpoint_path" in missing_params, "endpoint_path should be reported as missing"
